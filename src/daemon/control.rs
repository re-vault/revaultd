...
mod test {
    ...
    fn test_listvaults_from_db() {
        let mut revaultd = dummy_revaultd("test_listvaults", UserRole::ManagerStakeholder);
    ...
    fn test_vaults_from_deposits() {
        let mut revaultd =
            dummy_revaultd("test_vaults_from_deposits", UserRole::ManagerStakeholder);
        let db_file = revaultd.db_file();
        setup_db(&mut revaultd).unwrap();
        let vaults = create_vaults(&revaultd);
    ...
    fn test_presigned_txs() {
        let mut stake_revaultd = dummy_revaultd("test_presigned_txs", UserRole::Stakeholder);
        let man_revaultd = dummy_revaultd("test_presigned_txs", UserRole::Manager);
        ...
        fs::remove_dir_all(&stake_revaultd.data_dir).unwrap_or_else(|_| ());
    }

    #[test]
    fn test_finalized_emer_txs() {
        let mut revaultd = dummy_revaultd("test_finalized_emer_txs", UserRole::Manager);
        setup_db(&mut revaultd).unwrap();
        let db_file = revaultd.db_file();
        let vaults = create_vaults(&revaultd);
